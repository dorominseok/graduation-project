...
> 이 문서는 **실험 계획 + 진행 기록**을 함께 담는다. ① 분석 해설 실험은 2026-08-30~31 완료(7절). 운영 모델 `gemini-3.5-flash-lite` 확정.
...
| `gemini-3.6-flash` | thinking 기본 ON(케이스별 688~2,900 tok 선소모), 응답 7~29초. 무료 티어(별도 쿼터, 한도 낮음) | 4차 비교 완료 — 커버리지·지연·안정성 모두 lite에 뒤짐. 미채택 |
...
## 7. 실험 절차 및 결과 (① 분석 해설, 2026-08-30~31 완료)
...
| v3 | `gemini-3.6-flash` (4차) | 10/10 | 9 | 10 | 10 | 10 | 10 | 10 | 10 | 통제는 동급이나 **커버리지 결손 5/10**(부족 부위·불균형·신뢰도 누락), 지연 7~29초, 직렬화 결함 1건 |

**모델 비교** (프롬프트 v3 동일)

| | `gemini-3.6-flash` | `gemini-3.5-flash-lite` |
|---|---|---|
| 통제 4항목(V1 의미·V2·V3·V5) 위반 | 0 (표본 17) | 0 (표본 40+) |
| **커버리지 규칙 준수** (부족·과다 전부·불균형·LOW 언급) | **5/10 결손** — 한두 항목만 말하고 종료 | **결손 0** |
| thinking | 기본 ON, 688~2,236 tok 선소모, 끌 수 없음(v1beta) | 없음 |
| 숫자 직렬화 결함(7.4절) | **발생함** (4차 1건 + 08 의심) — "상위 모델이면 회피"는 성립 안 함 | **~7.5% / 호출** |
| 지연 | **7~29초** (변동 큼) | **~1.2초** |
| 무료 일일 쿼터 | 매우 낮음(~20회) — 1회 측정으로 소진 근접 | 별도 버킷, 상대적으로 여유 |
| **판정** | 느리고·커버리지 나쁘고·결함도 있음 | **운영 채택** |
...
| 대안 | ~~(a) `gemini-3.6-flash` 등 상위 모델~~ → **4차에서 flash도 결함 발생 확인. 모델 등급으로 못 피한다.** (b) 이상 응답(출력 토큰 급증·거대 수치·문장↔필드 불일치) 감지 시 1회 재요청 후 폴백, (c) flash-lite + 폴백 감수(~7.5%). **→ (b)+(c) 채택.** |
...
- **운영 모델 확정: `gemini-3.5-flash-lite`** + "이상 응답 1회 재요청 → 폴백". 4차(`gemini-3.6-flash` v3) 비교 결과 flash는 **커버리지 결손 5/10, 지연 7~29초, 직렬화 결함도 발생** — flash-lite가 통제 동급이면서 모든 축에서 우세.
- **직렬화 결함은 모델 등급으로 못 피한다** (flash-lite ~7.5%, flash도 발생). → 검증(V1) + 규칙 폴백이 유일한 방어선.
- **10월 ② 리스크 평가**: LLM을 규칙 안에 가두는 것은 가능. 단 소형·상위 모델 모두 구조화 출력에 직렬화 결함이 있으므로, ②도 정량 재검증 + 폴백을 필수로 둔다(8절).
- 남은 것: 문장 자연스러움은 지인 검증에서 정성 평가, 화면 배치는 9월 3주 통계 화면 설계에서.
...
| 운영 모델 | **`gemini-3.5-flash-lite`** — 통제 동급, 지연 1.2초, 비용 0, 커버리지·안정성 flash보다 우세 (4차 확인) | **확정** |
...
- **flash vs lite 비교 완료** (4차, 2026-08-31). `gemini-3.6-flash`는 v3 프롬프트에서 커버리지 결손 5/10, 지연 7~29초, 직렬화 결함 1건 — 운영 모델은 `gemini-3.5-flash-lite`로 확정. 상세는 `scripts/llm-experiment/runs/실험일지.md` 4차.
