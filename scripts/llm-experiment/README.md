...
`docs/LLM_연동_실험기록.md` 7절의 실험을 자동으로 돌린다. Gemini 무료 티어 사용.
...
| 파일 | 역할 |
|---|---|
| `analysis.py` | 부위별 주당 세트 수 → 8.2/8.3 응답 JSON 생성. `API_명세서.md` 8절 판정 로직의 참조 구현 |
...
| V6 | claims 1~6개, 각 120자 이내 (순수 길이). 커버리지 결손은 `[coverage]` note로 별도 표기 | 0개 / 6개 초과 / 120자 초과 |
| V7 | 구조화 출력이 스키마대로 파싱됨 | 형식 불안정 |

호출 자체 실패(타임아웃·차단·429/503 재시도 후 실패)와 숫자 직렬화 결함(V1이 탐지)은 폴백 문장으로 대체된다.

## 결과를 문서에 반영

`results.md`의 표를 `docs/LLM_연동_실험기록.md` 7.3절 "프롬프트 버전별" 표에 붙여넣고,
...
- 실제 호출은 네 Gemini 무료 쿼터를 소모한다(비용 0). 신형 3.x flash는 무료 일일 한도가 낮아(~20~25회) 429가 잦다. 태평양시 자정 리셋.
- 무료 티어는 프롬프트가 Google 모델 학습에 쓰일 수 있다. 여기 입력은 전부 합성 데이터라 무방하지만,
  실제 사용자 데이터로 옮길 때는 재검토(실험기록 10절).
